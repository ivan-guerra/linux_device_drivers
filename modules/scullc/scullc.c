/*
 * scullc.c - scullc ('c' for cache) char device implementation.
 *
...
static const struct proc_ops scullc_proc_ops = {
	.proc_open = scullc_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = seq_release,
};

/* proc entry pointer, see scullc_create_proc() and scullc_remove_proc(). */
...
{
	entry = proc_create("scullcmem", 0, NULL, &scullc_proc_ops);
}
...
struct scullc_dev *scullc_follow(struct scullc_dev *dev, int n)
{
	while (n--) {
		if (!dev->next) {
			dev->next =
				kmalloc(sizeof(struct scullc_dev), GFP_KERNEL);
...
int scullc_init(void)
{
	 ...
	 */
	scullc_devices =
		kmalloc(scullc_devs * sizeof(struct scullc_dev), GFP_KERNEL);
	if (!scullc_devices) {
		result = -ENOMEM;
		goto fail_malloc;
	}
